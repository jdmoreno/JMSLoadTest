...
public class CSVReader {
	...
	private static void readRecord(CSVRecord record) {
		...
		JMSDefRecord jmsDefRecord = JMSDefRecord.builder()
			...
			.isTopic(isTopic)
			.destName(destName)
			.useUniqueDests(useUniqueDests)
			.useXA(useXA)
			.txnSize(txnSize)
			.useProducer(useProducer)
			.countProducer(countProducer)
			.runTime(runTime)
			.threadsProducer(threadsProducer)
			.payloadFile(payloadFile)
			.useCompression(useCompression)
			.msgRate(msgRate)
			.msgSize(msgSize)
			.useConsumer(useConsumer)
			.countConsumer(countConsumer)
			.threadsConsumer(threadsConsumer)
			.deliveryMode(deliveryMode)
			.useAsync(useAsync)
			.selector(selector)
			.ackMode(ackMode).build();
			...
			if (isProducer) {
				if (jmsDefRecord.isUseProducer()) {
					JMSProducerParams jmsProducerParams = JMSProducerParams.builder()
						.serverUrl(jmsDefRecord.getServerUrl())
						.username(jmsDefRecord.getUsername())
						.password(jmsDefRecord.getPassword())
						.connections(jmsDefRecord.getConnections())
						.destName(jmsDefRecord.getDestName())
						.factoryName(jmsDefRecord.getFactoryName())
						.useTopic(jmsDefRecord.isTopic())
						.uniqueDests(jmsDefRecord.isUseUniqueDests())
						.xa(jmsDefRecord.isUseXA())
						.runTime(jmsDefRecord.getRunTime())
						.threads(jmsDefRecord.getThreadsProducer())
						.count(jmsDefRecord.getCountProducer())
						.payloadFile(jmsDefRecord.getPayloadFile())
						.compression(jmsDefRecord.isUseCompression())
						.msgRate(jmsDefRecord.getMsgRate())
						.txnSize(jmsDefRecord.getTxnSize())
						.msgSize(jmsDefRecord.getMsgSize())
						.deliveryMode(jmsDefRecord.getDeliveryMode())
						.async(jmsDefRecord.isUseAsync())
						.build();
				...
				if (jmsDefRecord.isUseConsumer()) {
					JMSConsumerParams jmsConsumerParams = JMSConsumerParams.builder()
						...
						.uniqueDests(jmsDefRecord.isUseUniqueDests())
						.xa(jmsDefRecord.isUseXA())
						.selector(jmsDefRecord.getSelector())
						.txnSize(jmsDefRecord.getTxnSize())
						.count(jmsDefRecord.getCountConsumer())
						.runTime(jmsDefRecord.getRunTime())
						.threads(jmsDefRecord.getThreadsConsumer())
						.ackMode(jmsDefRecord.getAckMode())
						.build();
					log.info(jmsConsumerParams.toString());	
					jmsConsumerParams.initialise();
					new JMSConsumer(jmsConsumerParams, null);
